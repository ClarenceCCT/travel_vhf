...
my_orig_country <- c("ET") # using 2-letter country code
...
## IMPORT DATA
##################################################################################
eth_iata <- map_dfr(sub_files, ~ import_iata(.x, origcodes = my_orig_airports, destcodes = my_dest_airports))
eth_iata <- process_iata(eth_iata)

##################################################################################
## SAVE DATA
##################################################################################
save_filename <- paste(save_path_rad, "eth_arrivals.rds", sep = "/")
saveRDS(eth_iata, file = save_filename)
